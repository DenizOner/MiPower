...
class StepHandler(ConfigFlowHandlerInterface):
    ...
    def __init__(
        ...
        try:
            ...
            # Get logging decorators through facade to maintain pattern
            if container:
                logger_facade = container.get("logger_facade")
                if logger_facade:
                    self._handler_method_logging = (
                        logger_facade.get_handler_method_logging_decorator()
                    )
                else:
                    # Fallback
                    ...
                    self._handler_method_logging = no_op_decorator
            else:
                # Fallback
                def no_op_decorator(func):
                    return func
    ...
    async def _convert_device_id_to_entity_id(self, device_id: str) -> Optional[str]:
        ...
        try:
            ...
            _LOGGER.debug(f"Power entity ID obtained: {result}")
            _LOGGER.info(
    ...
    async def _discover_scripts_for_device(self, remote_entity_id: str) -> List[Any]:
