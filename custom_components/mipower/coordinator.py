...
from homeassistant.const import STATE_ON  # type: ignore[import]
from homeassistant.core import Event, HomeAssistant, callback  # type: ignore[import]
...
class MiPowerCoordinator(DataUpdateCoordinator[bool], ICoordinator):
    ...
    async def async_setup(self) -> None:
        ...
        _LOGGER.debug(
            "[%s (%s)] Setting up coordinator.", name, self._media_player_entity_id
        )
    ...
    async def async_unload(self) -> None:
        ...
        name = self._get_media_player_name()
        _LOGGER.debug(
            "[%s (%s)] Unloading coordinator.", name, self._media_player_entity_id
        )
