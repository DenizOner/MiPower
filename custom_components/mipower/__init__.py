...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug("[%s (%s)] Hass.data structure for domain '%s' initialized.", name, entry.entry_id, DOMAIN)
    ...
    _LOGGER.debug("[%s (%s)] Retrieved media_player_entity_id: %s", name, entry.entry_id, media_player_entity_id)
    ...
    _LOGGER.debug("[%s (%s)] MiPowerCoordinator instance created.", name, entry.entry_id)
    ...
    _LOGGER.debug("[%s (%s)] Forwarded setup to platforms: %s", name, entry.entry_id, PLATFORMS)

    # Add an update listener. This function will be called when the user
    # changes the integration's options in the UI.
    entry.async_on_unload(entry.add_update_listener(_update_listener))
    _LOGGER.debug("[%s (%s)] Added update listener for options flow.", name, entry.entry_id)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug("[%s (%s)] Unloaded platforms with result: %s", name, entry.entry_id, unload_ok)
    ...
    if unload_ok:
        ...
        _LOGGER.debug("[%s (%s)] Popped coordinator from hass.data.", name, entry.entry_id)

        # Unload the coordinator itself to stop its listeners.
        await coordinator.async_unload()
        _LOGGER.debug("[%s (%s)] Coordinator unloaded successfully.", name, entry.entry_id)
