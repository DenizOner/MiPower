"""Integration logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in integration operations,
...
class IntegrationLogger:
    """Logger for integration operations with detailed tracking."""
...
def integration_logging(
    ...
    logger_name: str = "integration",
) -> Callable[[F], F]:
    """Decorator for comprehensive integration function logging.
    ...
    def decorator(func: F) -> F:
        logger = IntegrationLogger(logger_name)
...
def integration_entry_logging(entry_operation: str) -> Callable[[F], F]:
    """Decorator for integration entry operations.

    Args:
        entry_operation: Type of entry operation (initialize/cleanup)
    ...
    def context_provider():
        return {"operation": entry_operation, "type": "integration_entry"}

    return integration_logging(
        context_provider=context_provider, logger_name="integration.entries"
    )
...
def facade_method_logging(facade_name: str) -> Callable[[F], F]:
    ...
    return integration_logging(
        context_provider=context_provider, logger_name=f"integration.{facade_name}"
    )


def initializer_method_logging(method_name: str) -> Callable[[F], F]:
    """Decorator for initializer methods.

    Args:
        method_name: Name of the initializer method
    ...
    def context_provider():
        return {"method": method_name, "operation": "initializer_method"}

    return integration_logging(
        context_provider=context_provider,
        logger_name=f"integration.initializer.{method_name}",
    )
