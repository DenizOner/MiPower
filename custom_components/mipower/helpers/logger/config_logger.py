"""Config logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in config operations,
...
class ConfigLogger:
    """Logger for config operations with detailed tracking."""

    def __init__(self, logger_name: str = "config"):
        """Initialize the config logger.
...
def config_logging(
    ...
    """Decorator for comprehensive config function logging.
    ...
    def decorator(func: F) -> F:
        logger = ConfigLogger(logger_name)
...
def config_loader_logging(loader_name: str) -> Callable[[F], F]:
    """Decorator for config loader methods.

    Args:
        loader_name: Name of the config loader
    ...
    def context_provider():
        return {"loader": loader_name, "operation": "config_loading"}

    return config_logging(
        context_provider=context_provider, logger_name="config.loader"
    )


def config_merger_logging(merger_name: str) -> Callable[[F], F]:
    """Decorator for config merger methods.

    Args:
        merger_name: Name of the config merger
    ...
    def context_provider():
        return {"merger": merger_name, "operation": "config_merging"}

    return config_logging(
        context_provider=context_provider, logger_name="config.merger"
    )


def registry_config_loader_logging(loader_type: str) -> Callable[[F], F]:
    """Decorator for registry config loader methods.

    Args:
        loader_type: Type of the registry config loader
    ...
    def context_provider():
        return {"loader_type": loader_type, "operation": "registry_config_loading"}

    return config_logging(
        context_provider=context_provider, logger_name="config.registry"
    )


def config_validator_logging(validator_name: str) -> Callable[[F], F]:
    """Decorator for config validator methods.

    Args:
        validator_name: Name of the config validator
    ...
    def context_provider():
        return {"validator": validator_name, "operation": "config_validation"}

    return config_logging(
        context_provider=context_provider, logger_name="config.validator"
    )
