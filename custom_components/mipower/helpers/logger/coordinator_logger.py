"""Coordinator logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in coordinator operations,
...
class CoordinatorLogger:
    """Logger for coordinator operations with detailed tracking."""

    def __init__(self, logger_name: str = "coordinator"):
        """Initialize the coordinator logger.
...
def coordinator_logging(
    context_provider: Optional[Callable[[], Dict[str, Any]]] = None,
    logger_name: str = "coordinator",
) -> Callable[[F], F]:
    """Decorator for comprehensive coordinator function logging.
    ...
    def decorator(func: F) -> F:
        logger = CoordinatorLogger(logger_name)
...
def coordinator_method_logging(coordinator_name: str) -> Callable[[F], F]:
    """Decorator for coordinator methods.
    ...
    def context_provider():
        return {"coordinator": coordinator_name, "operation": "coordinator_method"}

    return coordinator_logging(
        context_provider=context_provider, logger_name=f"coordinator.{coordinator_name}"
    )


def configuration_manager_method_logging(manager_name: str) -> Callable[[F], F]:
    """Decorator for configuration manager methods.
    ...
    def context_provider():
        return {"manager": manager_name, "operation": "configuration_manager_method"}

    return coordinator_logging(
        context_provider=context_provider, logger_name=f"coordinator.{manager_name}"
    )


def event_handler_method_logging(handler_name: str) -> Callable[[F], F]:
    ...
    def context_provider():
        return {"handler": handler_name, "operation": "event_handler_method"}

    return coordinator_logging(
        context_provider=context_provider, logger_name=f"coordinator.{handler_name}"
    )
