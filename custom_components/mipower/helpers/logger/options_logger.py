"""Options flow logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in options flow operations,
...
class OptionsLogger:
    """Logger for options flow operations with detailed tracking."""

    def __init__(self, logger_name: str = "options"):
        """Initialize the options flow logger.
...
def options_logging(
    ...
    logger_name: str = "options",
) -> Callable[[F], F]:
    """Decorator for comprehensive options function logging.
    ...
    def decorator(func: F) -> F:
        logger = OptionsLogger(logger_name)
...
def options_step_logging(step_name: str) -> Callable[[F], F]:
    """Decorator for options flow step functions.

    Args:
        step_name: Name of the options flow step
    ...
    def context_provider():
        return {"step": step_name, "operation": "options_flow_step"}

    return options_logging(
        context_provider=context_provider, logger_name="options.steps"
    )
...
def facade_method_logging(facade_name: str) -> Callable[[F], F]:
    ...
    return options_logging(
        context_provider=context_provider, logger_name=f"options.{facade_name}"
    )
...
def handler_method_logging(handler_name: str) -> Callable[[F], F]:
    ...
    return options_logging(
        context_provider=context_provider, logger_name=f"options.{handler_name}"
    )
