"""DI module logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in dependency injection operations,
...
class DiLogger:
    """Logger for dependency injection operations with detailed tracking."""

    def __init__(self, logger_name: str = "di"):
        """Initialize the DI logger.
...
def di_logging(
    ...
    """Decorator for comprehensive DI function logging.
...
def container_resolve_logging(interface_name: str) -> Callable[[F], F]:
    """Decorator for container resolve methods.

    Args:
        interface_name: Name of the interface being resolved
    ...
    def context_provider():
        return {"interface": interface_name, "operation": "container_resolve"}

    return di_logging(context_provider=context_provider, logger_name="di.container")


def lazy_loader_property_logging(property_name: str) -> Callable[[F], F]:
    """Decorator for lazy loader properties.

    Args:
        property_name: Name of the property being loaded
    ...
    def context_provider():
        return {"property": property_name, "operation": "lazy_loading"}

    return di_logging(context_provider=context_provider, logger_name="di.lazy_loader")


def strategy_resolve_logging(strategy_name: str) -> Callable[[F], F]:
    """Decorator for resolution strategy methods.

    Args:
        strategy_name: Name of the resolution strategy
    ...
    def context_provider():
        return {"strategy": strategy_name, "operation": "strategy_resolve"}

    return di_logging(context_provider=context_provider, logger_name="di.strategies")
