"""Config flow logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in config flow operations,
...
class ConfigFlowLogger:
    """Logger for config flow operations with detailed tracking."""

    def __init__(self, logger_name: str = "config_flow"):
        """Initialize the config flow logger.

        Args:
            logger_name: Name for the logger instance
        """
        self.logger = logging.getLogger(f"smartify.{logger_name}")
...
def config_flow_logging(
    context_provider: Optional[Callable[[], Dict[str, Any]]] = None,
    logger_name: str = "config_flow",
) -> Callable[[F], F]:
    """Decorator for comprehensive config flow function logging.
    ...
    def decorator(func: F) -> F:
        logger = ConfigFlowLogger(logger_name)
...
def config_flow_step_logging(step_name: str) -> Callable[[F], F]:
    """Decorator for config flow step functions.

    Args:
        step_name: Name of the config flow step
    ...
    def context_provider():
        return {"step": step_name, "operation": "config_flow_step"}

    return config_flow_logging(
        context_provider=context_provider, logger_name="config_flow.steps"
    )
...
def facade_method_logging(facade_name: str) -> Callable[[F], F]:
    ...
    return config_flow_logging(
        context_provider=context_provider, logger_name=f"config_flow.{facade_name}"
    )
...
def handler_method_logging(handler_name: str) -> Callable[[F], F]:
    ...
    return config_flow_logging(
        context_provider=context_provider, logger_name=f"config_flow.{handler_name}"
    )


def schema_builder_logging(builder_name: str = "schema_builder") -> Callable[[F], F]:
    """Decorator for schema builder methods.

    Args:
        builder_name: Name of the schema builder
    ...
    def context_provider():
        return {"builder": builder_name, "operation": "schema_building"}

    return config_flow_logging(
        context_provider=context_provider, logger_name=f"config_flow.{builder_name}"
    )


def validator_logging(validator_name: str = "validator") -> Callable[[F], F]:
    """Decorator for validator methods.
    ...
    def context_provider():
        return {"validator": validator_name, "operation": "validation"}

    return config_flow_logging(
        context_provider=context_provider, logger_name=f"config_flow.{validator_name}"
    )


def discovery_service_logging(service_name: str = "discovery") -> Callable[[F], F]:
    """Decorator for discovery service methods.

    Args:
        service_name: Name of the discovery service
    ...
    def context_provider():
        return {"service": service_name, "operation": "device_discovery"}

    return config_flow_logging(
        context_provider=context_provider, logger_name=f"config_flow.{service_name}"
    )
