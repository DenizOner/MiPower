"""Switch logging decorator for detailed error tracking.

This module provides decorators for comprehensive logging in switch operations,
...
class SwitchLogger:
    """Logger for switch operations with detailed tracking."""

    def __init__(self, logger_name: str = "switch"):
...
def switch_logging(
    ...
    logger_name: str = "switch",
) -> Callable[[F], F]:
    """Decorator for comprehensive switch function logging.
    ...
    def decorator(func: F) -> F:
        logger = SwitchLogger(logger_name)
...
def switch_operation_logging(operation_name: str) -> Callable[[F], F]:
    """Decorator for switch operation functions.

    Args:
        operation_name: Name of the switch operation
    ...
    def context_provider():
        return {"operation": operation_name, "type": "switch_operation"}

    return switch_logging(
        context_provider=context_provider, logger_name="switch.operations"
    )


def switch_service_logging(service_name: str) -> Callable[[F], F]:
    """Decorator for switch service methods.

    Args:
        service_name: Name of the switch service
    ...
    def context_provider():
        return {"service": service_name, "type": "switch_service"}

    return switch_logging(
        context_provider=context_provider, logger_name=f"switch.{service_name}"
    )


def switch_entity_logging(entity_name: str) -> Callable[[F], F]:
    """Decorator for switch entity methods.

    Args:
        entity_name: Name of the switch entity
    ...
    def context_provider():
        return {"entity": entity_name, "type": "switch_entity"}

    return switch_logging(
        context_provider=context_provider, logger_name=f"switch.{entity_name}"
    )
